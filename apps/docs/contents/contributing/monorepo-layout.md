...
| `@sealant/web`         | Main product app — sandboxes, execution records, SSH-key settings, and preview repository/profile/registry views. TanStack Start + TanStack Router.                                                                   | `apps/web/src/routes/_authenticated/*`, `apps/web/DESIGN.md`                           |
...
| `@sealant/db`                  | Shared PostgreSQL + Drizzle persistence for the control plane — sandbox lifecycle, telemetry/execution records, auth, and source-integration tables.                                                | `packages/db/src/client.ts`, `packages/db/src/repositories/*`                                    |
