...
| `@sealant/worker`      | Background job worker: consumes workspace build jobs, compiles/publishes images via BuildKit, launches workspace runtimes, updates durable lifecycle state.                                                             | `apps/worker/src/index.ts`, `apps/worker/src/runtime-env.ts`                           |
...
| `@sealant/sdk`                 | The fluent public SDK (`Sealant`, `opencode()` harness) — create a workspace, run a harness, replay the record. **Unpublished** (`"private": true`); consumable only from inside this monorepo today. | `packages/sdk/src/client.ts`, `packages/sdk/src/facade/*`                                            |
