...
Sealant is self-hosted. You run the whole stack — web app, API, SSH gateway, build worker, and the
control-plane database — on your own infrastructure with Docker Compose. This page installs it on a
single host.
...
6. **Writes `~/.config/sealant/.env`** with mode `0600`, pins `SEALANT_VERSION`, persists the
   selected API/web/SSH ports and bind host, and **generates any missing infrastructure secrets**
   once (the database password, the SSH gateway token, and the auth secret). Existing values are
   left untouched.
...
Each port is configurable (`SEALANT_WEB_PORT`, `SEALANT_API_PORT`, `SEALANT_SSH_PORT`), and the bind
host is `SEALANT_BIND_HOST`. To reach Sealant from another machine, see
[Beyond localhost](/docs/guides/beyond-localhost). The full list of knobs is in
