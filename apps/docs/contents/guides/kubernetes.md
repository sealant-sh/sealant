...
- **Unprivileged user namespaces enabled on build nodes** (`user.max_user_namespaces > 0`) —
  ...
  quota, not disk). On Talos: `machine.sysctls: { user.max_user_namespaces: "15000" }`.
...
- Workspace-scoped Docker (`tooling.services.docker`) is refused on Kubernetes until the DinD
  sidecar ships as its own capability.
