...
- **For workspace-scoped Docker** (`workspaces.docker.enabled`, off by default): Docker-enabled
  ...
  Docker-enabled workspaces (workspaces without Docker are unaffected). On Ubuntu 23.10+ nodes,
  AppArmor's user-namespace restriction (`kernel.apparmor_restrict_unprivileged_userns=1`) stops the
  rootless daemon from creating its own namespace inside the Pod
  (`rootlesskit … fork/exec /proc/self/exe: operation not permitted`); set it to `0` or grant
...
| Docker-enabled Pod stays Pending, or `container 'docker' CrashLoopBackOff` | The launch error quotes the sidecar's log tail. `rootlesskit … fork/exec /proc/self/exe: operation not permitted` is Ubuntu's AppArmor userns restriction (prerequisites above). `FailedCreatePodSandBox … mounting "sysfs" … operation not permitted` in `kubectl describe` means the node cannot create user-namespaced Pods at all (nested containerd such as kind on GitHub-hosted runners; containerd < 2.0). A mount error on the store claim means its filesystem is not idmap-capable (NFS). |
