...
description:
  A field-by-field walk through the workspace builder — source, dotfiles, harness, runtime, image
  target, packages, commands, and SSH — plus how to manage workspaces afterward.
---

A workspace is a live, disposable development environment built around a real repository. You create
one from the builder at `/workspaces/new`. The form is a single spec: as you fill it in, a live JSON
preview and health checks update on the right, and submitting it kicks off a real build that
redirects you to the workspace detail page.
...
- **Setup Steps** — an ordered list of commands run while building the workspace (install extra
  tooling, prepare state). Optional.
- **Entrypoint Command** — an optional command that becomes the workspace's entrypoint.

## Security & Access

Toggle **Enable SSH tunneling** to make the workspace reachable over the SSH gateway. If you have no
...
SSH is authorized by workspace owner, so you do not attach specific keys here — any key you have
...
- **Raw manifest (JSON)** — the exact `spec` that will be submitted. This is the same shape returned
  later at `/workspaces/<workspace-id>/spec`.
...
- **`/workspaces`** lists your workspaces with total / running / failed metrics and rows showing
  name, id, repository, tag, status, and created time. **`/workspaces/active`** and
...
stop/restart/expire methods are typed but not implemented. If you need to clean up a workspace
today, bring its container down through Docker on the host. For working with the run inside a
workspace and its record, see [Runs and execution records](/docs/guides/runs-and-execution-records).

## Related

- [GitHub App](/docs/guides/github-app) — connect private repositories.
- [SSH access](/docs/guides/ssh-access) — keys and connecting.
- [Workspaces](/docs/concepts/workspaces) — the concept and lifecycle.
- [HTTP API](/docs/reference/http-api) — creating workspaces programmatically.
