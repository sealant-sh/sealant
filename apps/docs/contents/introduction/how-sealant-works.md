...
description:
  The services a self-hosted Sealant install runs, and how a sandbox request flows through them.
---
...
| ----------------- | -------------------------------------------------------------------------------------------------------------------------------- | -------------------------------- |
| Web app           | The product UI: sign-up, sandbox creation, sandbox detail, SSH key management.                                                   | `http://localhost:3000`          |
| Control-plane API | The HTTP API everything talks to: sandboxes, runs, SSH keys, GitHub installations. Serves `/openapi.json` and docs at `/docs`.   | `http://localhost:4000`          |
| Worker            | Background service that picks up sandbox build jobs, builds the sandbox image, publishes it, and launches the sandbox container. | internal                         |
| SSH gateway       | Public-key SSH entry point that routes `ssh sbx-<sandbox-id>@…` sessions into the right live sandbox.                            | `localhost:2222`                 |
| Zot registry      | An OCI image registry that stores built sandbox images.                                                                          | `127.0.0.1:5000` (loopback only) |
| Postgres          | Control-plane state: users, sandboxes, runs, SSH keys, events.                                                                   | internal only                    |
...
One important detail: the worker mounts the host Docker socket. Sandbox containers and images are
...
1. **You submit a spec.** In the web app (or via the [HTTP API](/docs/reference/http-api)) you
   describe the sandbox: the repository (raw Git URL or a GitHub App repository), a ref, the
...
2. **The API normalizes and persists it.** The control-plane API validates the spec, stores the
   sandbox in Postgres, and enqueues a build job on RabbitMQ.
3. **The worker builds.** The worker leases the job, clones the repository (minting a short-lived
   ...
   sandbox.
4. **The image is published.** The built image is pushed to the bundled zot registry, so the sandbox
   is reproducible from a pinned image reference and digest.
5. **The sandbox launches.** The worker starts the sandbox container on the host Docker daemon and
   reports runtime status, endpoints, and lifecycle events back through the API. The web app's
   sandbox detail page shows attempts, events, runtime state, and the published image.
6. **You connect.** With SSH enabled and a key registered, `ssh -p 2222 sbx-<sandbox-id>@localhost`
   reaches the SSH gateway. The gateway verifies your public key against the API (key fingerprint →
   owning user), checks that you own the sandbox, and proxies your session into the running
   container. VS Code and Cursor Remote-SSH ride the same path
   ([SSH access](/docs/guides/ssh-access)).
7. **Harness executions become runs.** A run started inside a sandbox produces an execution record —
...
- The API and SSH gateway share a generated secret (`SANDBOX_SSH_GATEWAY_TOKEN`) for the internal
...
- [Your first sandbox](/docs/getting-started/first-sandbox)
