...
| Web app: sandbox flow  | Shipped | Create, list, rename, rerun, and inspect sandboxes (status, attempts, events, spec, published image). No stop/delete from the UI yet.                                                                                |
| SSH access             | Shipped | Register keys in Settings → SSH keys, then `ssh -p 2222 sbx-<sandbox-id>@localhost`; VS Code/Cursor Remote-SSH work over the same path. See [SSH access](/docs/guides/ssh-access).                                   |
| GitHub App integration | Shipped | Import an installation at `/github/setup`, sync repositories, and use private repos in sandbox creation. See [GitHub App](/docs/guides/github-app).                                                                  |
| HTTP API               | Shipped | Sandboxes, runs and execution records (timeline/scrollback/loss/changes), SSH keys, registries, GitHub, package resolution; OpenAPI at `/openapi.json`. See [HTTP API](/docs/reference/http-api).                    |
...
| SDK (`@sealant/sdk`)   | Preview | Real TypeScript code covering the sandbox → run → record path, but unpublished (`"private": true`) — usable only inside the monorepo today. See [SDK](/docs/reference/sdk).                                          |
| CLI                    | Planned | No `sealant` CLI exists. The installer script is the only command-line entry point. See [CLI](/docs/reference/cli).                                                                                                  |
| Runs review UI         | Preview | The web review surface is the sandbox detail page (attempts, events, spec, image). No dedicated run timeline/diff/artifact UI in the web app yet; the record is readable via the API.                                |
| Issue workflows        | Preview | The `/issues` board renders; Linear OAuth import brings in real issues, but seeded rows are static and drag/drop state does not persist. No issue-workflow API. See [Issue workflows](/docs/guides/issue-workflows). |
