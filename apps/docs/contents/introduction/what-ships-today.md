...
| Web app: workspace flow    | Shipped | Create, list, rename, rerun, stop, and inspect workspaces (status, build history, events, spec, published image). No hard delete from the UI (stopped workspaces stay in the record).                                                                                                                   |
| Workspace lifecycle        | Shipped | Stop (`POST /v1/workspaces/:id/stop`), restart, and TTL expiry are wired end to end: the worker removes the container, records the terminal `stopped` state with a reason, and a reaper stops expired/stranded runtimes. Default TTL via `SEALANT_WORKSPACE_DEFAULT_TTL_SECONDS`.                       |
...
| SDK (`@sealant/sdk`)       | Shipped | Published on npm as `@sealant/sdk`. It covers workspace create/get/list, lifecycle (`stop()` / `restart()` / `expire()`), server-side `harness.run()` / `harness.start()`, run lookup, and record reads. `harness.session()` still throws `SealantNotImplementedError`. See [SDK](/docs/reference/sdk). |
...
| Registry UI                | Mock    | `/registry` pages still read mock data through the web app's registry service. The real registry endpoints exist in the [HTTP API](/docs/reference/http-api); on a single-host install they report the local Docker Engine image store.                                                                 |
