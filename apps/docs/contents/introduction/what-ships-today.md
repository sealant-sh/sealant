...
| SDK (`@sealant/sdk`)   | Preview | Covers the sandbox → run → record path: blocking `harness.run()`, non-blocking `harness.start()` + `record.stream()`, and the record read surface. First npm release in flight — usable only inside the monorepo until it lands. See [SDK](/docs/reference/sdk). |
| CLI                    | Planned | No `sealant` CLI exists. The installer script is the only command-line entry point. See [CLI](/docs/reference/cli).                                                                                                                                              |
| Runs review UI         | Preview | The run record page (sandbox → run) renders the recorded timeline: commands with per-command scrollback, attribution, and raw events. No artifact UI yet; live runs poll rather than stream.                                                                     |
