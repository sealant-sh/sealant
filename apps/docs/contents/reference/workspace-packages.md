...
- **A pinned upstream release**, where the family's repositories have no package, or lag the
  ...
  binaries to `/usr/local/bin`. The version and the checksum move together, by pull request to
  Sealant.
...
Every entry was built and run on every family, on ARM64 Lambda MicroVMs, on 2026-09-21.
