...
| `WORKSPACE_IMAGE_RETAINED_PLANS`        | `3`                                      | Worker only. Distinct build plans whose newest image is kept with no workspace on it, so an unchanged plan is reused by hash instead of rebuilt.                                         |
