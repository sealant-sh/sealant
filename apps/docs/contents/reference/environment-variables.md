...
| `WORKSPACE_RUNTIME_EXIT_POLL_INTERVAL_MS` | `5000`                                   | Worker only. How often each runtime is asked whether its `ready` workspaces are still up; one that died on its own is recorded `failed` with its exit code. Docker also reports exits as they happen. |
| `SEALANT_DOCKER_HOST_GATEWAY_ALIAS`       | `true`                                   | Worker only. Give every Docker workspace `host.docker.internal` for the host (`--add-host …:host-gateway`), so it can reach services on the host by name on Linux. `false` omits the entry.           |
