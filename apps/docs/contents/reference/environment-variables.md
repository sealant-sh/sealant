...
| `SEALANT_K8S_DOCKER_ENABLED`                                                                       | `false`                                   | Serve `services.docker` as a rootless dind sidecar in a user-namespaced Pod (chart: `workspaces.docker.enabled`). Off: such blueprints are refused at launch.                       |
