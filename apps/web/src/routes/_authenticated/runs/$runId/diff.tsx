    ...
    >
      <div className="border border-border">
        {[
          ...
          <div key={path} className="grid gap-2 border-b border-border px-4 py-3 last:border-b-0 sm:grid-cols-[auto_1fr_auto] sm:items-center">
            <span className="font-mono text-[0.62rem] uppercase tracking-[0.13em] text-muted-foreground">{kind}</span>
            <p className="font-mono text-xs text-foreground">{path}</p>
            <p className="font-mono text-[0.62rem] uppercase tracking-[0.13em] text-muted-foreground">{delta}</p>
          </div>
        ))}
      </div>
    </RunDetailSection>
