...
const OCI_RUNTIME_OPTIONS: ReadonlyArray<{
  ...
  },
  {
    value: "runsc",
    label: "runsc",
    detail: "gVisor-isolated Docker runtime for stronger workspace boundaries.",
  },
];
...
function NewWorkspacePage() {
  ...
  const [form, setForm] = useState<NewWorkspaceFormState>(() => {
  ...
  // Gateway SSH access authorizes by workspace OWNER, so the create flow only needs to make sure
  // the user has at least one registered key — the key is account-level, not per-workspace.
  ...
  useEffect(() => {
    for (const validationState of packageValidationStates) {
      ...
      console.error("[new-workspace] package validation failed", {
  ...
  const setField = <TField extends keyof NewWorkspaceFormState>(
    field: TField,
    value: NewWorkspaceFormState[TField],
  ) => {
    setForm((current) => {
  ...
  const handleSubmit = async (event: FormEvent<HTMLFormElement>) => {
    ...
    try {
      ...
      await queryClient.invalidateQueries({ queryKey: trpc.workspace.list.pathKey() });
      window.location.assign(`/workspaces/${encodeURIComponent(response.workspaceId)}`);
    } catch (error) {
      ...
      <div className="grid min-h-[calc(100svh-9.5rem)] gap-0 xl:grid-cols-[1.45fr_0.65fr]">
        ...
        >
          <header className="border-b border-rule-faint px-8 py-10 sm:px-10">
            <p className="ev-eyebrow">New workspace</p>
            ...
            <p className="mt-3 max-w-3xl text-sm leading-7 text-muted-foreground">
              Configure operational parameters for a new workspace and submit one typed launch
                  ...
                  >
                    <div>
                      ...
                      <p className="mt-1 text-xs leading-6 text-muted-foreground">
                        Allow direct terminal access to the active workspace runtime endpoint.
                      </p>
                    </div>
                    <span
                      className={
                    ...
                    sshKeysQuery.isLoading ? (
                      ...
                      <div className="rounded-2xl border border-warning-dot/40 bg-background px-4 py-4">
                        <p className="text-sm font-medium text-warning">
                          No SSH keys registered — add one to connect to this workspace
                        </p>
                        <p className="mt-1 text-xs leading-6 text-muted-foreground">
                          Paste your public key (usually{" "}
                          <span className="font-mono">~/.ssh/id_ed25519.pub</span>). It registers to
                          your account, so it works for every workspace you own. You can also add
              ...
              <div className="border-t border-rule-faint px-8 py-6 sm:px-10">
                <div className="border-l-2 border-danger-dot py-1 pl-3">
                  <p className="text-sm font-medium text-danger">
                    Fix the fields below before starting the workspace
                  </p>
        ...
        <aside className="border-t border-rule-faint bg-card px-6 py-8 sm:px-8 xl:border-t-0">
          <div className="xl:sticky xl:top-6">
            ...
            <div className="mt-10 space-y-3 border-t border-rule-faint pt-6">
              ...
              >
                {createWorkspaceMutation.isPending ? "Starting workspace..." : "Start workspace"}
              </Button>
              <Button
                type="button"
                variant="outline"
...
function createInitialFormState(registryId: string): NewWorkspaceFormState {
  return {
    sourceMode: "git",
...
function resolveErrorMessage(error: unknown): string {
  ...
  return "Unable to create workspace right now.";
}
