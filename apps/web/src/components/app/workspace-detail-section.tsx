...
interface WorkspaceDetailSectionProps {
  ...
  if (workspace === null) {
    return (
      <WorkspacePage
        ...
        description="The selected workspace does not exist in the current workspace view."
      />
    );
  }
    ...
    >
      <div className="space-y-8">
        <RunRecordPanel
          ...
          rows={[
            { label: "Workspace", value: workspace.workspaceId },
            { label: "Repository", value: workspace.repository ?? "Unknown" },
            { label: "Tag", value: workspace.tag ?? "n/a" },
          ]}
        />
        {children}
      </div>
    ...
    <div className="overflow-hidden rounded-2xl border border-border bg-card shadow-[var(--shadow-sm)]">
      <div className="flex items-center justify-between gap-3 border-b border-rule-faint px-6 py-4">
        <span className="inline-flex items-center gap-2.5">
          ...
          <span className="font-mono text-xs text-ink-2">workspace · {recordId}</span>
        </span>
