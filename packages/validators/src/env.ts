...
// Compat shim for the sandbox → workspace rename: composes generated by pre-rename installers
// still pass the sandbox-era names, and 0.5.0 turned that into a boot-time validation crash on
// in-place upgrades. An old name fills in only where its new name is absent. Remove once released
// composes predating the rename are out of circulation.
const renamedEnvAliases: ReadonlyArray<readonly [oldName: string, newName: string]> = [
  ["SANDBOX_SSH_GATEWAY_TOKEN", "WORKSPACE_SSH_GATEWAY_TOKEN"],
  ["SANDBOX_SSH_GATEWAY_HOST", "WORKSPACE_SSH_GATEWAY_HOST"],
  ["SANDBOX_SSH_GATEWAY_PORT", "WORKSPACE_SSH_GATEWAY_PORT"],
  ["SANDBOX_SSH_GATEWAY_USERNAME_PREFIX", "WORKSPACE_SSH_GATEWAY_USERNAME_PREFIX"],
  ["SSH_GATEWAY_SANDBOX_USERNAME_PREFIX", "SSH_GATEWAY_WORKSPACE_USERNAME_PREFIX"],
  ["SANDBOX_CONTROL_SOCKET_HOST_DIR", "WORKSPACE_CONTROL_SOCKET_HOST_DIR"],
];

const applyRenamedEnvAliases = (input: NodeJS.ProcessEnv): NodeJS.ProcessEnv => {
  const output = { ...input };

  for (const [oldName, newName] of renamedEnvAliases) {
    const oldValue = output[oldName];
    const newValue = output[newName];

    // Empty string counts as absent, matching the parsers' emptyStringAsUndefined.
    if ((newValue === undefined || newValue === "") && oldValue !== undefined && oldValue !== "") {
      output[newName] = oldValue;
    }
  }
...
export const parseAppEnv = (input: NodeJS.ProcessEnv): AppEnv => {
  const runtimeEnv = createEnv({
    ...
    runtimeEnv: applyRenamedEnvAliases(input),
