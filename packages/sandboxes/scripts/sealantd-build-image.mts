/**
 * Standalone Fedora sandbox image build, with sealantd baked in.
 *
 * This drives the BuildKit OS builder via its pure compile entry point
 * (`compileSandboxBuildSpec`) and intentionally avoids the worker / DB / queue
 ...
 *
 * Flow:
 *   1. Build a SandboxBlueprint (validated via parseSandboxBlueprint).
 *   2. compileSandboxBuildSpec renders the build context, runs `docker build`
 ...
 *
 * Run with:  DOCKER_BUILDKIT=1 pnpm --filter @sealant/sandboxes exec tsx scripts/sealantd-build-image.mts
 * (or from repo root: DOCKER_BUILDKIT=1 node_modules/.bin/tsx packages/sandboxes/scripts/sealantd-build-image.mts)
 */
import { spawn } from "node:child_process";

import { parseSandboxBlueprint } from "@sealant/validators";
...
const main = async () => {
  // harness.id MUST be one of the validator's sandboxHarnessIdSchema enum
