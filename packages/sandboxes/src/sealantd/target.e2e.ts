 ...
 * `execInSandbox` helper drives a REAL control session (P5 `SealantRuntime` over the docker-exec
 ...
 *      `-v <hostTmp-with-.git>:/sandbox/repo`. Every other docker call (inspect/etc.) passes through
...
import { execInSandbox, sealantTargetForDockerContainer } from "./target.js";
...
describe.skipIf(!imageAvailable)(
  "P6 seam: DockerRuntimeAdapter container id -> SealantTarget -> execInSandbox (real container)",
  () => {
    ...
    beforeAll(async () => {
      ...
      const mountingRunner: DockerCommandRunner = async (command, args) => {
        ...
        // Insert `-v <mount>:/sandbox/repo` just before the trailing image reference.
      ...
      const launchInput = parseRuntimeAdapterLaunchInput({
        ...
        publishedImage: {
          repository: "sealant-sandbox-fedora-claude-code",
          tag: "claude-code",
    ...
    it("container-id -> sealantTargetForDockerContainer -> execInSandbox runs /bin/echo hi", async () => {
      ...
      const result = await Effect.runPromise(
        execInSandbox(target, { executable: "/bin/echo", args: ["hi"] }).pipe(
