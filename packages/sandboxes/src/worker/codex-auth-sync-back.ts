...
/*
Codex auth.json sync-back (design doc §2 codex / §6). The official Codex CLI inside the sandbox
...
Concurrency note: the safe model is one live copy per credential. Concurrent sandboxes sharing a
...
export interface SyncBackCodexAuthJsonInput<R = never> {
  ...
  /**
   * Reads the sandbox's current `$HOME/.codex/auth.json` contents (e.g. an exec into the
...
/**
 * Best-effort persistence of a sandbox's refreshed codex auth.json after a run completes.
 *
...
export const syncBackCodexAuthJson = Effect.fn("syncBackCodexAuthJson")(function* <R = never>(
  ...
  const rawAuthJson = yield* input
    ...
    .pipe(
      Effect.catch((cause) =>
        warn(
          "Codex auth sync-back skipped: could not read $HOME/.codex/auth.json from the sandbox (the container may already be gone).",
          cause,
  ...
  if (!parsed.valid) {
    yield* warn(`Codex auth sync-back skipped: sandbox auth.json is invalid (${parsed.reason}).`);
    return;
  }
  ...
  for (const codexRef of codexRefs) {
    ...
    const syncOne = Effect.gen(function* () {
      ...
      // fresher rotation since this sandbox launched.
