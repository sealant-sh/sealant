...
import { assertWorkspaceBuildQueueTopology } from "./topology.js";
...
const createRabbitMqSingleton = async (connectionUrl: string): Promise<RabbitMqSingleton> => {
  ...
  await assertWorkspaceBuildQueueTopology(publishChannel);
  await assertWorkspaceBuildQueueTopology(consumeChannel);
