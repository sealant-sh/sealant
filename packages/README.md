...
- `jobs`: business-agnostic Postgres-backed job queue (pg-boss) shared by the API and worker
- `workspaces`: workspace domain package covering build, publish, deploy, queue wiring, and
  lifecycle mapping, including harness integrations and package resolution/standardization
