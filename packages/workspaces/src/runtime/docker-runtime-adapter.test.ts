...
describe("DockerRuntimeAdapter runtime observation", () => {
  it("inspects containers: running, exited with the exit code and a log tail, missing", async () => {
    ...
    const inspections = await adapter.inspect(["c-running", "c-paused", "c-exited", "c-gone"]);

    expect(inspections.get("c-running")).toEqual({ state: "running" });
    expect(inspections.get("c-paused")).toEqual({ state: "running" });
    expect(inspections.get("c-exited")).toEqual({
      state: "exited",
      exitCode: 137,
      detail: "status: exited, exitCode: 137\nLogs:\nlast words",
    });
    expect(inspections.get("c-gone")).toEqual({ state: "missing" });
  ...
  it("surfaces a daemon failure from inspect instead of guessing a state", async () => {
    ...
    await expect(adapter.inspect(["c-1"])).rejects.toThrow(/Cannot connect/);
  });
  ...
  it("reports die events for its own containers and reconnects when the stream drops", async () => {
    ...
    try {
      ...
      expect(onExit.mock.calls).toEqual([[{ resourceId: "aaa", exitCode: 137 }]]);
      ...
      expect(onExit).toHaveBeenLastCalledWith({ resourceId: "ddd", exitCode: 0 });
