 ...
 * real workspace image built by the BuildKit OS builder (P3-Build).
 *
 ...
 *        - A host dir containing a `.git/` marker is bind-mounted at `/workspace/repo` (the resolved
 ...
 * unit run (`*.test.ts`) and runs via `pnpm --filter @sealant/workspaces test:e2e`.
 */
...
describe("sealantd baked-image e2e proof", () => {
  ...
  beforeAll(async () => {
    // A host dir with a `.git/` marker; bind-mounted at /workspace/repo to skip the entrypoint clone.
